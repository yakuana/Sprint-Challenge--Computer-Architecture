...
class CPU:
    ...
    def run(self):
        ...
        while running:
            ...
            if IR == 0b00000001: # HLT
                ...
                return_address = self.pc + 2                # push return address on stack
                self.reg[SP] -= 1                           # decrement stack pointer 
                self.ram[self.reg[SP]] = return_address
                reg_num = self.ram[self.pc + 1]             # set the pc to the value in the register
                self.pc = self.reg[reg_num]

            elif IR == 0b00010001: # RET
                self.pc = self.ram[self.reg[SP]]            # pop the return address off stack
                self.reg[SP] += 1                           # store it in the pc
    ...
    def ram_write(self):
        # address to read and value stored at address 
